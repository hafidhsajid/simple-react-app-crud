...
class ReadorderCustomer extends React.Component {
  // Constructors
  constructor(props) {
  ...
  componentDidMount() {
    ...
    // console.log(tokens);
    fetch(
      `${process.env.REACT_APP_HOST}member/order/history`,
            ...
            <tbody>
              {items.map((item) => (
                <tr>
                  ...
                  <td>
                    {" "}
                    <Link to={`/customer/order/${item.OrderId}?id=${item.OrderId}`}>
                    <button
                      type="button"
...
export default ReadorderCustomer;
